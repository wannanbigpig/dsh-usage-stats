 ...
 * or an `assistant/message` (`data.usage`); current `assistant/attempt` logs
 * contribute the last usage in each independent embedded stream. A sample for the same
...
function sampleOf(event) {
	if (event.type === "assistant/attempt") {
		...
		for (let index = stream.length - 1; index >= 0; index -= 1) {
			...
			if (record?.type !== "chunk" || record.chunk?.type !== "usage") continue;
			const usage = record.chunk.usage;
			if (usage === null || typeof usage !== "object") continue;
			// Several attempts may settle in one step; seq identifies each call.
			return { key: Number.isInteger(event.seq) ? `attempt:${event.seq}` : null, usage };
		}
		return void 0;
	}
...
function beijingHourStart(day, hour) {
	if (typeof day !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(day) || !Number.isInteger(hour) || hour < 0 || hour > 23) return null;
	const [year, month, date] = day.split("-").map(Number);
	const timestamp = Date.UTC(year, month - 1, date, hour - 8);
	const roundTrip = new Date(timestamp + BEIJING_OFFSET_MS);
	return roundTrip.getUTCFullYear() === year && roundTrip.getUTCMonth() === month - 1 && roundTrip.getUTCDate() === date && roundTrip.getUTCHours() === hour
		? timestamp
		: null;
}

/** Resolve the price-table model for a request model at one exact instant. */
export function billingModelIdOf(modelKey, pricingConfig = {}, atMs = null) {
	const model = modelIdOf(modelKey);
	if (atMs === null || atMs === void 0 || !Number.isFinite(Number(atMs))) return model;
	const at = Number(atMs);
	let selected = null;
	for (const route of pricingConfig.routes ?? []) {
		if (route?.model !== model) continue;
		const from = Date.parse(route.effectiveFrom);
		const until = route.effectiveUntil === void 0 ? Number.POSITIVE_INFINITY : Date.parse(route.effectiveUntil);
		if (!Number.isFinite(from) || !Number.isFinite(until) && until !== Number.POSITIVE_INFINITY || at < from || at >= until) continue;
		if (selected === null || from > selected.from) selected = { from, priceModel: route.priceModel };
	}
	return selected?.priceModel ?? model;
}
 ...
 */
export function priceOf(modelKey, pricingConfig = {}, atMs = null) {
	const pricing = pricingConfig.pricing ?? {};
	const model = billingModelIdOf(modelKey, pricingConfig, atMs);
 ...
 */
export function costOf(modelKey, buckets, hour, pricingConfig = {}, day = null, attributionAt = null) {
	if (!isBilledModelKey(modelKey)) return null;
	const billingAt = attributionAt !== null && attributionAt !== void 0 && Number.isFinite(Number(attributionAt)) ? Number(attributionAt) : beijingHourStart(day, hour);
	const billingModel = billingModelIdOf(modelKey, pricingConfig, billingAt);
	const price = priceOf(modelKey, pricingConfig, billingAt);
	if (price === null) return null;
	const explicit = pricingConfig.models?.[billingModel];
 ...
 */
export function costNanosOf(modelKey, buckets, hour, pricingConfig = {}, day = null, attributionAt = null) {
	if (!isBilledModelKey(modelKey)) return null;
	const billingAt = attributionAt !== null && attributionAt !== void 0 && Number.isFinite(Number(attributionAt)) ? Number(attributionAt) : beijingHourStart(day, hour);
	const billingModel = billingModelIdOf(modelKey, pricingConfig, billingAt);
	const basePrice = priceOf(modelKey, pricingConfig, billingAt);
	if (basePrice === null) return null;
	const explicit = pricingConfig.models?.[billingModel];
	const peak = isPeakBillingPeriod(day, hour, pricingConfig);
	const explicitTier = peak ? explicit?.peak : explicit?.offPeak;
	const effectivePrice = explicitTier && priceOf(`${providerOf(modelKey)}/${billingModel}`, { pricing: { [billingModel]: explicitTier } }) !== null
		? explicitTier
		: basePrice;
	const terms = [
