 ...
 *
 * Each captured call freezes its nano-CNY cost before its ledger/archive
 * update queues for sequential row writes. Session persistence is used only
 * for an explicit estimated-history rebuild.
 *
...
import { clearLedgerState, freezeLedgerEntry, pricingVersionOf, recordLedgerState, renderLedgerState, renderWorkspaceUsage, trimLedgerState } from "./ledger.js";
import { queryDeepSeekBalance } from "./balance.js";
import { providerErrorStatusOf } from "./provider-status.js";
...
import { readSessionTitlesFromPersistence, rebuildEstimatedFromPersistence } from "./rebuild.js";
...
const inject = ["credentials", "settings", "storageDomain", "connection", "webServer", "sessionPersistence"];
...
export function validateConfig(raw = {}) {
	...
	if (raw.pricing !== void 0) {
		...
		// Preserve versioned rates through the host schema -> apply validation
		// round-trip. Derived legacy mirrors must never replace explicit peaks.
		if (raw.pricing.models !== void 0) {
			validatePricingInput(raw.pricing);
			pricing = normalizePricing(raw.pricing);
		} else {
			const legacyRates = structuredClone(raw.pricing.pricing ?? {});
			if (raw.pricing.peakMultiplier !== void 0) {
				if (merged.peakMultiplier <= 0) throw new Error("config.pricing.peakMultiplier must be positive");
				for (const [model, defaults] of Object.entries(merged.pricing)) {
					const row = legacyRates[model] ?? {};
					legacyRates[model] = { ...row, peak: Object.fromEntries(["inputMiss", "inputHit", "output"].map(field => [field,
						row.peak?.[field] ?? (numberOrNull(row[field]) ?? defaults[field]) * merged.peakMultiplier])) };
				}
			}
			pricing = normalizePricing({ ...raw.pricing, pricing: legacyRates });
		}
	}
...
export function createProviderService({ ctx, config, settingsService, credentials, deps = {} }) {
	...
	return {
		...
		refreshAll: async (force = true) => {
			const providers = await getProviders();
			return Promise.all(providers.filter((entry) => entry.queryable && entry.configured && entry.id !== "deepseek-official").map((entry) => get(entry.id, entry.keyRef, force)));
		},
...
export async function configuredKeys(ctx, config) {
	...
	config = officialAccountConfig(ctx, config);
	const entries = [];
...
function officialAccountConfig(ctx, config) {
	const profile = serviceOf(ctx, "settings")?.get?.("llm-deepseek");
	const ref = nonEmptyString(profile?.apiKeyEnv) ?? config.defaultKeyRef;
	return { ...config, defaultKeyRef: ref, keys: [...new Set([ref, ...config.keys])],
		baseURL: nonEmptyString(profile?.baseURL) ?? config.baseURL };
}

export function createBalanceService({ ctx, credentials, config, settingsService, deps = {} }) {
	const cache = new Map();
	const inflight = new Map();
	...
	async function fetchBalance(ref, force = false) {
		const current = officialAccountConfig(ctx, config);
		const baseURL = ref === current.defaultKeyRef ? current.baseURL : config.baseURL;
		const cacheKey = JSON.stringify([ref, baseURL]);
		const hit = cache.get(cacheKey);
		const age = (deps.now ?? Date.now)() - (hit?.fetchedAt ?? 0);
		const refreshMs = settingsService?.snapshot?.().refreshMs ?? config.refreshMs;
		...
		const promise = (async () => {
			...
			try {
				const raw = await (deps.queryBalance ?? queryDeepSeekBalance)(baseURL, apiKey, deps.timeoutMs ?? UPSTREAM_TIMEOUT_MS, deps.fetch);
	...
	return {
		get: fetchBalance,
		refreshAll: (force = true) => Promise.all(officialAccountConfig(ctx, config).keys.map((ref) => fetchBalance(ref, force))),
		cached: (ref) => {
			const current = officialAccountConfig(ctx, config);
			return cache.get(JSON.stringify([ref, ref === current.defaultKeyRef ? current.baseURL : config.baseURL])) ?? null;
		}
	};
}

/** When enabled, refresh immediately and repeat balance + local usage every N ms.
...
export function startBackgroundRefresh(ctx, balanceService, config, deps = {}) {
	...
	const run = async (force = false) => {
		...
		if (!force && !(Number(getRefreshMs()) > 0)) return;
		running = true;
		active = (async () => {
			const results = await Promise.allSettled([balanceService.refreshAll(force), refreshProvider(force), collectUsage(ctx, getPricing())]);
...
async function evaluateStatuses({ ctx, config, balanceService, limits, usage, today, deps, keyRef, refreshBalances = true }) {
	...
	const keys = keyRef ? [keyRef] : [...new Set([...config.keys, ...Object.keys(limits.keys)])];
	...
	for (const ref of keys) {
		const rule = resolveLimitRule(limits, ref);
		if (!rule.enabled || (rule.lowBalanceWarning === null && rule.minBalance === null)) continue;
		const now = (deps.now ?? Date.now)();
		...
		if (refreshBalances && !cacheFresh && typeof balanceService?.get === "function") refreshTargets.push(ref);
	}
...
function resolveLimitRule(allLimits, keyRef) {
	const activeRule = rule => ({ ...rule, dailyCostLimit: rule.period === "monthly" ? null : rule.dailyCostLimit, monthlyCostLimit: rule.period === "monthly" ? rule.monthlyCostLimit : null });
	const global = allLimits?.global?.enabled === true ? activeRule(allLimits.global) : defaultLimitRule();
	const configuredKeyRule = allLimits?.keys?.[keyRef];
	if (configuredKeyRule?.enabled !== true) return global;
	const keyRule = activeRule(configuredKeyRule);
	...
	if (!global.enabled) return keyRule;
	...
	return {
		...
		dailyCostLimit: lowerPositive(keyRule.period === "monthly" ? null : keyRule.dailyCostLimit, global.period === "monthly" ? null : global.dailyCostLimit),
		monthlyCostLimit: lowerPositive(keyRule.period === "monthly" ? keyRule.monthlyCostLimit : null, global.period === "monthly" ? global.monthlyCostLimit : null),
...
function evaluateKeyQuota({ keyRef, limits, todayCost = 0, todayCostReliable = true, monthlyCost = 0, monthlyCostReliable = true, balance = null, balanceStatus, balanceFetchedAt, now = Date.now(), balanceMaxAgeMs = Infinity }) {
	...
	// Both scopes may contribute a different billing period. Choose the most
	// urgent reliable cap for presentation without dropping the other period.
	const candidates = [
		{ monthly: false, limit: rule.dailyCostLimit, cost: numericCost, reliable: todayCostReliable !== false },
		{ monthly: true, limit: rule.monthlyCostLimit, cost: numericMonthlyCost, reliable: monthlyCostReliable !== false }
	].filter(item => item.limit !== null && item.limit > 0);
	candidates.sort((left, right) => Number(right.reliable) - Number(left.reliable) || right.cost / right.limit - left.cost / left.limit);
	const useMonthlyLimit = candidates[0]?.monthly ?? rule.period === "monthly";
	...
	const spendStatus = spendLimit !== null && spendLimit > 0
		...
		: "muted";
	const costLimitReached = spendCostReliable && spendLimit !== null && spendLimit > 0 && spendCost >= spendLimit;
	...
	return {
		...
		period: useMonthlyLimit ? "monthly" : "daily",
		lowBalanceWarning: rule.lowBalanceWarning,
		minBalance: rule.minBalance,
		alertPercent: rule.alertPercent,
		currentBalance: numericBalance,
...
/**
 * Append one call-level ledger entry and await its sequential row writes.
 * When the ledger exceeds `deps.maxLedgerEntries` (default 5000), the oldest
 * entries are folded into the exact frozen archive by the same queued update.
 * The host persists each row independently; this is not a multi-row transaction.
 * Only imported official entries without a historical price remain estimated.
...
function createLimitsService({ ctx, config, balanceService, deps = {} }) {
	...
	async function updateLimits(raw) {
		...
		if (body === null || (!isRecord(body.global) && !isRecord(body.keys)) || (Object.hasOwn(body, "global") && !isRecord(body.global)) || (Object.hasOwn(body, "keys") && !isRecord(body.keys))) {
	...
	async function evaluateStatus(keyRef) {
		...
		const currentConfig = officialAccountConfig(ctx, config);
		const settings = typeof deps.settings?.load === "function" ? await deps.settings.load() : deps.settings?.get?.();
		currentConfig.refreshMs = settings?.refreshMs ?? config.refreshMs;
		const ref = keyRef || currentConfig.defaultKeyRef;
		const usage = await (deps.collectUsage ?? collectUsage)(ctx, await runtimePricing());
		const today = (deps.todayKey ?? todayKeyLocal)();
		const { statuses } = await evaluateStatuses({ ctx, config: currentConfig, balanceService, limits, usage, today, deps, keyRef: ref });
		const dayEntry = (usage.days ?? []).find((d) => d.date === today);
	...
	async function evaluateAll() {
		...
		const today = (deps.todayKey ?? todayKeyLocal)();
		const currentConfig = { ...officialAccountConfig(ctx, config), refreshMs: settings.refreshMs ?? config.refreshMs };
		const { statuses, globalTodayCost } = await evaluateStatuses({ ctx, config: currentConfig, balanceService, limits, usage, today, deps, refreshBalances: refreshCadenceOf(settings, config) > 0 });
	...
	async function check(payload = {}) {
		...
		const targetKey = byProvider
			...
			?? officialAccountConfig(ctx, config).defaultKeyRef;
...
function runtimePricingOf(settings, config) {
	if (settings?.pricing !== null && settings?.pricing !== undefined) return normalizePricing(settings.pricing);
...
function workspaceDateBound(value, field) {
	if (value === void 0 || value === null || value === "") return null;
	const text = String(value);
	if (!/^\d{4}-\d{2}-\d{2}$/.test(text)) throw new UsageRpcBadRequestError(`${field} must be YYYY-MM-DD`);
	return text;
}
...
export function createUsageOperations({ ctx, config, settingsService, repository, balanceService, providerService, limitsService, deps = {} }) {
	/** Per-session rebuild fold cache; revisions are instance-scoped, so this dies with the service. */
	const rebuildFoldCache = new Map();
	/** Per-session title cache over the same persistence-service instance. */
	const sessionTitleReadCache = new Map();
	const accountPayload = async () => {
		...
		const automaticRefreshDisabled = refreshCadenceOf(settings, config) <= 0;
		const accounts = {};
		for (const provider of providers) {
			...
			accounts[provider.id] = {
				...(cached ?? { id: provider.keyRef ?? provider.id, status: provider.queryable ? (automaticRefreshDisabled ? "refresh-disabled" : "pending") : "unsupported", fetchedAt: null, capabilities: provider.capabilities }),
				providerId: provider.id,
		...
		return { ok: true, keys, defaultKeyRef: officialAccountConfig(ctx, config).defaultKeyRef, accounts, providers, defaultProviderId, settings: { refreshMs: settings.refreshMs, display: settings.display, maxLedgerEntries: maxLedgerEntriesOf(settings, config), defaultProviderId: settings.defaultProviderId ?? defaultProviderId, visibleProviderIds } };
	};

	const evaluatedLimits = async () => {
		const evaluated = await limitsService.evaluateAll();
		return { ok: true, limits: evaluated.limits, status: evaluated.statuses, defaultKeyRef: officialAccountConfig(ctx, config).defaultKeyRef, todayCost: evaluated.todayCost, alerts: evaluated.alerts };
	};
	const effectiveNotifications = (settings) => ({
		...settings.notifications,
		channels: { ...settings.notifications?.channels, sidebar: settings.display?.statusDot !== false && settings.notifications?.channels?.sidebar !== false }
	});

	return {
		"usage/get": async (payload) => {
			const revision = repository.revision?.() ?? null;
			const settings = await settingsService.load();
			...
			return { ok: true, ...usage, revision, today: dayKey(Date.now()), pricing: { currency: pricing.currency, peakHours: pricing.peakHours, peakMultiplier: pricing.peakMultiplier, weekendOffPeakFrom: pricing.weekendOffPeakFrom } };
		},
		/**
		 * Workspace to session token rollup for an inclusive Beijing-calendar date
		 * window. The exact ledger/frozen attribution and the session-persistence
		 * rebuild are disjoint, so both layers are summed without deduplication.
		 */
		"usage/workspaces": async (payload, signal) => {
			const query = queryOf(payload);
			const from = workspaceDateBound(query.from, "from");
			const to = workspaceDateBound(query.to, "to");
			if (from !== null && to !== null && from > to) throw new UsageRpcBadRequestError("from must not be after to");
			const rendered = renderWorkspaceUsage(repository.get(), { from, to });
			const sessionIds = rendered.workspaces.flatMap((workspace) => workspace.sessions.map((session) => session.sessionId)).filter(Boolean);
			const persistedTitles = await readSessionTitlesFromPersistence(serviceOf(ctx, "sessionPersistence"), sessionIds, {
				signal,
				cache: sessionTitleReadCache,
				onError: (sessionId, error) => ctx.logger?.warn?.(`usage-stats: failed to read title for session "${sessionId}": ${String(error)}`)
			});
			const liveTitles = deps.sessionTitles instanceof Map ? deps.sessionTitles : new Map();
			return { ok: true, ...rendered, workspaces: rendered.workspaces.map((workspace) => ({
				...workspace,
				sessions: workspace.sessions.map((session) => {
					const title = liveTitles.get(session.sessionId) ?? persistedTitles.get(session.sessionId);
					return typeof title === "string" && title.trim() !== "" ? { ...session, title } : session;
				})
			})) };
		},
		"usage/revision": async () => ({ ok: true, revision: repository.revision?.() ?? null }),
		...
		"balance/get": async (payload) => {
			...
			const cacheOnly = !force && query.auto === "1" && refreshCadenceOf(await settingsService.load(), config) <= 0;
			if (providerId !== "" && providerId !== "deepseek-official") {
				if (!cacheOnly) return { ok: true, account: await providerService.get(providerId, requested || null, force) };
				const provider = (await providerService.providers()).find((entry) => entry.id === providerId);
				const ref = requested || provider?.keyRef || providerId;
				return { ok: true, account: providerService.cached?.(providerId, ref) ?? { id: ref, providerId, status: provider?.queryable ? "refresh-disabled" : "unsupported", fetchedAt: null, capabilities: provider?.capabilities ?? [] } };
			}
			const current = officialAccountConfig(ctx, config);
			const ref = requested !== "" && current.keys.includes(requested) ? requested : current.defaultKeyRef;
			return { ok: true, account: cacheOnly ? balanceService.cached?.(ref) ?? { id: ref, status: "refresh-disabled", fetchedAt: null } : await balanceService.get(ref, force) };
		},
		"limits/get": evaluatedLimits,
		"limits/update": async (payload) => {
			try { await limitsService.updateLimits(bodyOf(payload)); }
		...
		"alerts/get": async () => ({ ok: true, alerts: (await limitsService.evaluateAll()).alerts ?? [], notifications: effectiveNotifications(await settingsService.load()) }),
		"alerts/update": async (payload) => {
			...
			if (Object.hasOwn(notifications.channels ?? {}, "sidebar")) patch.display = { statusDot: notifications.channels.sidebar };
			const updated = await settingsService.update(patch);
			return { ok: true, notifications: effectiveNotifications(updated) };
		},
		...
		"data/rebuild-estimated": async (payload, signal) => {
			...
			const rebuilt = await rebuildEstimatedFromPersistence(serviceOf(ctx, "sessionPersistence"), baseline, { signal, cache: rebuildFoldCache });
...
function registerUsageInterceptors(ctx, config, settingsService, limitsService, deps) {
	...
	// Route bookkeeping for the durable assistant/attempt fallback: an attempt
	// event carries only turn/step/stream, so its provider/model comes from the
	// last request/header, or the stream call that produced it, or unknown.
	const lastHeaderBySession = new Map();
	const lastStreamRouteBySession = new Map();
	// Session header cwd (absolute working directory) powers the workspace rollup.
	const workspaceBySession = new Map();
	const sessionTitles = deps.sessionTitles instanceof Map ? deps.sessionTitles : new Map();
	// Sample identities outlive live metadata: an already captured call may
	// still be committing when the session leaves the host store.
	const forgetSession = (sessionId) => {
		if (sessionId === null) return;
		activeSteps.delete(sessionId);
		lastHeaderBySession.delete(sessionId);
		lastStreamRouteBySession.delete(sessionId);
		workspaceBySession.delete(sessionId);
		sessionTitles.delete(sessionId);
	};
	ctx.effect(() => ctx.on("session/disposed", (session) => {
		forgetSession(session?.id ?? session?.sessionId ?? null);
	}), "usage-stats: session disposal");
	const rememberBounded = (map, key, value) => {
		if (key === null) return;
		map.set(key, value);
		while (map.size > 10000) map.delete(map.keys().next().value);
	};
	const routeOfSession = (sessionId) => lastHeaderBySession.get(sessionId) ?? lastStreamRouteBySession.get(sessionId) ?? { provider: "unknown", model: "unknown" };
	// Prefer the live session header; fall back to the last cwd seen for this id.
	const workspaceOfSession = (sessionId, session = null) => {
		const direct = session?.header?.cwd;
		if (typeof direct === "string" && direct !== "") return direct;
		return sessionId === null ? null : workspaceBySession.get(sessionId) ?? null;
	};
	const lastUsageOfStream = (stream) => {
		if (!Array.isArray(stream)) return null;
		for (let index = stream.length - 1; index >= 0; index -= 1) {
			const record = stream[index];
			if (record?.type === "chunk" && record.chunk?.type === "usage" && record.chunk.usage !== null && typeof record.chunk.usage === "object") return record.chunk.usage;
		}
		return null;
	};
	...
	const sampleKeyOf = (sessionId, provider, model, turn, step, attempt = 0) => Number.isInteger(turn) && Number.isInteger(step)
		? `${sessionId ?? "<unscoped>"}:${provider ?? "unknown"}/${model ?? "unknown"}:${turn}:${step}${attempt > 0 ? `:attempt:${attempt}` : ""}`
		: null;
	ctx.effect(() => ctx.on("llm/stream", async function* (payload, next) {
		...
		rememberBounded(lastStreamRouteBySession, sessionId, { provider, model });
		...
		let sampleKey = sampleKeyOf(sessionId, provider, model, turn, step, activeStep?.attempt);
		...
		try {
			...
			if (usage !== null && !isFacadeProvider(provider)) {
				...
				const workspace = workspaceOfSession(sessionId);
				const entry = freezeLedgerEntry({ id: deps.createLedgerId?.() ?? randomUUID(), occurredAt: startedAt, completedAt, provider, model, ...(turn === null ? {} : { turn }), ...(step === null ? {} : { step }), sampleKey, ...(sessionId === null ? {} : { sessionId }), ...(workspace === null ? {} : { workspace }), usage }, runtimePricingOf(settingsService.snapshot(), config));
				try {
					...
					capturedStreamSamples.delete(sampleKey);
			...
			// The host appends assistant/message only after the stream is drained.
			// Keep successful captures authoritative through that later event;
			// bound auxiliary calls whose session never publishes a final message.
			while (capturedStreamSamples.size > 10000) capturedStreamSamples.delete(capturedStreamSamples.values().next().value);
		}
	...
	ctx.effect(() => ctx.on("session/event", async (session, event) => {
		...
		const eventWorkspace = session?.header?.cwd;
		if (sessionId !== null && typeof eventWorkspace === "string" && eventWorkspace !== "") rememberBounded(workspaceBySession, sessionId, eventWorkspace);
		if (event?.type === "session/title") {
			const title = event.data?.title;
			if (sessionId !== null && typeof title === "string" && title.trim() !== "") rememberBounded(sessionTitles, sessionId, title);
			return;
		}
		if (event?.type === "request/header") {
			const headerConfig = event.data?.header?.config;
			if (typeof headerConfig?.provider === "string" || typeof headerConfig?.model === "string") {
				rememberBounded(lastHeaderBySession, sessionId, {
					provider: typeof headerConfig.provider === "string" && headerConfig.provider !== "" ? headerConfig.provider : "unknown",
					model: typeof headerConfig.model === "string" && headerConfig.model !== "" ? headerConfig.model : "unknown"
				});
			}
			return;
		}
		if (event?.type === "step/start") {
			if (sessionId !== null && Number.isInteger(event.data?.turn) && Number.isInteger(event.data?.step)) {
				activeSteps.set(sessionId, { turn: event.data.turn, step: event.data.step, attempt: 0 });
		...
		if (event?.type === "assistant/attempt") {
			...
			const attempt = active?.turn === turn && active?.step === step ? active.attempt : 0;
			// Durable attempt-settlement fallback: when llm/stream did not observe
			// (or could not capture) this attempt, record the last usage embedded in
			// its stream so a failed/aborted call is billed without a rebuild.
			const usage = lastUsageOfStream(event.data?.stream);
			if (usage !== null) {
				const route = routeOfSession(sessionId);
				const sampleKey = sampleKeyOf(sessionId, route.provider, route.model, turn, step, attempt);
				if (sampleKey !== null && !isFacadeProvider(route.provider) && !capturedStreamSamples.has(sampleKey)) {
					const occurredAt = Number(event.time) || Date.now();
					const workspace = workspaceOfSession(sessionId, session);
					const entry = freezeLedgerEntry({ id: randomUUID(), occurredAt, completedAt: occurredAt, provider: route.provider, model: route.model, ...(turn === null ? {} : { turn }), ...(step === null ? {} : { step }), sampleKey, ...(sessionId === null ? {} : { sessionId }), ...(workspace === null ? {} : { workspace }), usage }, runtimePricingOf(settingsService.snapshot(), config));
					try {
						if (deps.recordLedger) await deps.recordLedger(entry);
						else await recordLedgerEntry(ctx, entry, { maxLedgerEntries: () => maxLedgerEntriesOf(settingsService.snapshot(), config), pricing: runtimePricingOf(settingsService.snapshot(), config), replaceSampleKey: true });
					} catch (error) { ctx.logger?.warn?.(`usage-stats: attempt ledger record failed: ${String(error)}`); }
				}
			}
			// A settled failed call owns its captured usage even if this step retries.
			if (active?.turn === turn && active?.step === step) active.attempt += 1;
			return;
		}
		...
		const active = sessionId === null ? null : activeSteps.get(sessionId);
		const attempt = active?.turn === turn && active?.step === step ? active.attempt : 0;
		let sampleKey = sampleKeyOf(sessionId, provider, model, turn, step, attempt);
		...
		if (capturedStreamSamples.has(sampleKey)) {
			if (unscopedStream?.completed === true) removeUnscopedStream(unscopedKey, unscopedStream);
			return;
		}
		const workspace = workspaceOfSession(sessionId, session);
		const entry = freezeLedgerEntry({ id: randomUUID(), occurredAt, completedAt: occurredAt, provider, model, ...(turn === null ? {} : { turn }), ...(step === null ? {} : { step }), sampleKey, ...(sessionId === null ? {} : { sessionId }), ...(workspace === null ? {} : { workspace }), usage: event.data.usage }, runtimePricingOf(settingsService.snapshot(), config));
		try {
			if (deps.recordLedger) await deps.recordLedger(entry);
			else await recordLedgerEntry(ctx, entry, { maxLedgerEntries: () => maxLedgerEntriesOf(settingsService.snapshot(), config), pricing: runtimePricingOf(settingsService.snapshot(), config), replaceSampleKey: true });
...
async function apply(ctx, rawConfig = {}, deps = {}) {
	...
	const balanceService = deps.balanceService ?? createBalanceService({ ctx, credentials, config, settingsService });
	...
	const sessionTitles = deps.sessionTitles instanceof Map ? deps.sessionTitles : new Map();
	const runtimeDeps = { ...deps, sessionTitles };
	const operations = createUsageOperations({ ctx, config, settingsService, repository, balanceService, providerService, limitsService, deps: runtimeDeps });
	registerUsageRpc(ctx, createUsageRpcDispatcher(operations, ctx.logger));
	registerUsageInterceptors(ctx, config, settingsService, limitsService, runtimeDeps);
	if (deps.disableBackgroundRefresh !== true) {
		const disposeRefresh = createBackgroundRefreshController({
			...
			start: () => startBackgroundRefresh(ctx, balanceService, config, { getRefreshMs: () => refreshCadenceOf(settingsService.snapshot(), config), getPricing: () => runtimePricingOf(settingsService.snapshot(), config), refreshProvider: (force) => providerService.refreshAll(force) })
		});
