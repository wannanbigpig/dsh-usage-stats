...
export function createUsageOperations({ ctx, config, settingsService, repository, balanceService, providerService, limitsService, deps = {} }) {
	const accountPayload = async () => {
		...
		return { ok: true, keys, defaultKeyRef: config.defaultKeyRef, accounts, providers, defaultProviderId, settings: { refreshMs: settings.refreshMs, display: settings.display, conversation: settings.conversation, maxLedgerEntries: maxLedgerEntriesOf(settings, config), defaultProviderId: settings.defaultProviderId ?? defaultProviderId, visibleProviderIds } };
	};
	...
	return {
		...
		"accounts/update": async (payload) => {
			...
			for (const field of ["display", "conversation"]) if (body[field] !== void 0) {
				if (body[field] === null || typeof body[field] !== "object" || Array.isArray(body[field])) throw new UsageRpcBadRequestError(`${field} must be an object`);
			...
			return { ok: true, defaultProviderId: updated.defaultProviderId, ...(credential ? { credential } : {}), settings: { refreshMs: updated.refreshMs, display: updated.display, conversation: updated.conversation, maxLedgerEntries: maxLedgerEntriesOf(updated, config), defaultProviderId: updated.defaultProviderId, visibleProviderIds: visibleProviderIdsOf(updated, providers, updated.defaultProviderId) } };
		},
		"pricing/get": async () => {
			const settings = await settingsService.load();
