 ...
 *   GET/POST /api/usage-stats/data     — data metadata (GET), clear/trim (POST)
 *
...
function validateSettings(raw = {}) {
	...
	if (raw.conversation !== null && typeof raw.conversation === "object" && !Array.isArray(raw.conversation)) {
		out.conversation = {
			...
			showTokenUsage: raw.conversation.showTokenUsage === true,
			showSessionTokenUsage: raw.conversation.showSessionTokenUsage === true
		};
	}
	return out;
}
...
async function handleAccounts(ctx, config, balanceService, providerService, settingsService, req, res) {
	...
	try {
		...
		if (body.conversation !== void 0) {
			...
			for (const field of ["enabled", "showTokenUsage", "showSessionTokenUsage"]) {
...
async function handleData(ctx, config, settingsService, req, res) {
	...
	try {
		...
		if (!["clear", "trim"].includes(action)) throw new TypeError("data action must be clear/trim");
		if (action === "clear") {
...
function apply(ctx, rawConfig = {}, deps = {}) {
	...
	// A raw llm stream has no durable event id. Keep bounded provisional samples
	// until assistant/message arrives; explicit turn/step keys are matched first,
	// while anonymous samples are only consumed when the candidate is unambiguous.
	const pendingUsageKeys = new Map();
	const PENDING_USAGE_TTL_MS = 10 * 60 * 1000;
	const MAX_PENDING_USAGE_PER_ROUTE = 64;
	const prunePendingUsage = (routeKey, now = Date.now()) => {
		const pending = pendingUsageKeys.get(routeKey) ?? [];
		const fresh = pending.filter((item) => now - item.at < PENDING_USAGE_TTL_MS);
		if (fresh.length === 0) pendingUsageKeys.delete(routeKey);
		else if (fresh.length !== pending.length) pendingUsageKeys.set(routeKey, fresh);
		return fresh;
	};
	const rememberPendingUsage = (routeKey, sampleKey, turn, step) => {
		const now = Date.now();
		const pending = prunePendingUsage(routeKey, now);
		pending.push({ sampleKey, at: now, explicit: Number.isInteger(turn) && Number.isInteger(step) });
		if (pending.length > MAX_PENDING_USAGE_PER_ROUTE) pending.splice(0, pending.length - MAX_PENDING_USAGE_PER_ROUTE);
		pendingUsageKeys.set(routeKey, pending);
	};
	const takePendingUsage = (routeKey, explicitSampleKey) => {
		const pending = prunePendingUsage(routeKey);
		let index = explicitSampleKey === null ? -1 : pending.findIndex((item) => item.sampleKey === explicitSampleKey);
		if (index < 0) {
			const anonymous = pending.filter((item) => item.explicit !== true);
			// A single anonymous candidate can still be correlated to an explicit
			// event; multiple candidates are ambiguous and must remain provisional.
			if (anonymous.length === 1) index = pending.indexOf(anonymous[0]);
			else if (explicitSampleKey === null && pending.length === 1) index = 0;
		}
	...
	if (typeof ctx.on === "function") {
					...
					if (usage !== null && usage !== void 0 && !isFacadeProvider(ledgerProvider)) {
						try {
							if (sampleKey !== null) {
								...
								rememberPendingUsage(routeKey, sampleKey, usageTurn, usageStep);
							}
		...
		ctx.effect(() => ctx.on("session/event", async (session, event) => {
			...
			const eventTurn = Number.isInteger(event.data?.turn) ? event.data.turn : null;
			const eventStep = Number.isInteger(event.data?.step) ? event.data.step : null;
			const explicitSampleKey = eventTurn !== null && eventStep !== null ? `${routeKey}:${eventTurn}:${eventStep}` : null;
			const matchedPending = takePendingUsage(routeKey, explicitSampleKey);
			const sampleKey = matchedPending?.sampleKey ?? explicitSampleKey ?? `${routeKey}:event:${Date.now()}:${randomUUID()}`;
			try {
				...
				const entry = freezeLedgerEntry({
					...
					...(eventTurn !== null ? { turn: eventTurn } : {}),
					...(eventStep !== null ? { step: eventStep } : {}),
					sampleKey, usage: event.data.usage
				}, runtimePricingOf(settingsService.snapshot(), config));
				if (typeof deps.recordLedger === "function") await deps.recordLedger(entry);
				else await recordLedgerEntry(ctx, entry, { maxLedgerEntries: () => maxLedgerEntriesOf(settingsService.snapshot(), config) });
			} catch (error) {
