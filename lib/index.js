...
/**
 * Resolve official Harness provider directory entries without copying the
 * catalog into plugin settings. User-declared aliases and wrapper routes are
 * intentionally excluded even when they are active model routes.
 */
export async function configuredProviders(ctx, config, settingsSnapshot = null) {
	...
	const isOfficialConfigEntry = (entry) => entry?.provider === "deepseek-official" || entry?.declared === false;
	try {
	...
	const officialConfigIds = new Set(configurableEntries.filter(isOfficialConfigEntry).map((entry) => entry.provider));
	if (activeProviderIds !== null) {
		for (const entry of configurableEntries) {
			if (activeProviderIds.has(entry?.provider) && isOfficialConfigEntry(entry)) add(entry);
		}
		for (const entry of builtins.values()) {
			if (activeProviderIds.has(entry.id) && (entry.official === true || officialConfigIds.has(entry.id))) add(entry);
		}
		for (const id of activeProviderIds) {
			if (!entries.has(id) && (id === "deepseek-official" || builtins.get(id)?.official === true)) add({ provider: id, displayName: id });
		}
	} else if (configurableEntries.length > 0) {
		// Older hosts may expose only the catalog. Use the settings profile to
		// identify routes that are actually configured; never copy the catalog.
		for (const entry of configurableEntries) {
			...
			if (profile !== null && typeof profile === "object" && !Array.isArray(profile) && isOfficialConfigEntry(entry)) add(entry);
		}
	}
	if (activeProviderIds === null || activeProviderIds.has("deepseek-official")) {
	...
	for (const entry of entries.values()) {
		...
		result.push({
			...
			baseURL,
			capabilities,
			queryable: builtins.has(entry.id) || entry.id === "deepseek-official",
...
function alertEventCategoryOf(status = {}) {
	if (status?.reason === "min_balance") return "lowBalance";
...
async function handleAccounts(ctx, config, balanceService, providerService, settingsService, req, res) {
	...
	try {
		if (req.method === "GET") {
			...
			for (const ref of config.keys) {
				const account = typeof balanceService?.cached === "function" ? balanceService.cached(ref) : null;
				accounts[ref] = account ?? { id: ref, status: "pending", fetchedAt: null };
			}
			for (const provider of providers) {
				if (provider.id === "deepseek-official") continue;
				const account = providerService.cached(provider.id, provider.keyRef);
				if (account !== null) accounts[provider.id] = account;
			}
			json(res, 200, {
				ok: true,
				keys,
...
async function handlePricing(ctx, config, settingsService, req, res) {
	...
	try {
		...
		if (body?.action === "restore") {
			...
			updated = await settingsService.update({ pricing: normalizePricing(body.pricing) });
		} else {
