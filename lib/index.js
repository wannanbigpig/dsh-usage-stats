 ...
 *
 * Registers eight loopback-only endpoints on the web server:
...
export function startBackgroundRefresh(ctx, balanceService, config, deps = {}) {
	...
	const run = async () => {
		...
		running = true;
		active = (async () => {
			const results = await Promise.allSettled([balanceService.refreshAll(), collectUsage(ctx, getPricing())]);
...
async function handleAccounts(ctx, config, balanceService, settingsService, req, res) {
	if (rejectForeignCaller(req, res, ["GET", "POST"])) return;
	try {
		if (req.method === "GET") {
			const settings = await settingsService.load();
			...
			json(res, 200, {
				...
				accounts,
				settings: { refreshMs: settings.refreshMs, display: settings.display, conversation: settings.conversation }
			});
			return;
		}
		const body = await readJsonBody(req);
		...
		json(res, 200, { ok: true, settings: { refreshMs: updated.refreshMs, display: updated.display, conversation: updated.conversation } });
	} catch (error) {
...
async function handleData(ctx, config, settingsService, req, res) {
	...
	try {
		...
		if (action === "clear") {
			// 危险动作：前端必须二次确认；后端独立校验 action 字段，防误触发。
...
function apply(ctx, rawConfig = {}, deps = {}) {
	...
	ctx.effect(() => ctx.webServer.register({
		kind: "exact",
		path: ACCOUNTS_PATH,
		handler: (req, res) => handleAccounts(ctx, config, balanceService, settingsService, req, res)
	...
	if (deps.disableBackgroundRefresh !== true) ctx.effect(() => startBackgroundRefresh(ctx, balanceService, config, {
		getRefreshMs: () => settingsService.snapshot().refreshMs ?? config.refreshMs,
