...
const LedgerEntryBaseSchema = z.object({
	...
	// Session/workspace attribution is optional: entries recorded before the
	// dimension existed stay valid, and the session-persistence rebuild covers
...
/** Compacted token attribution for one workspace; workspace totals are derived by summing its sessions. */
const FrozenWorkspaceSchema = z.object({
	sessions: z.record(z.string(), UsageBucketsSchema)
}).strict();
export const FrozenDaySchema = z.object({
	totals: UsageBucketsSchema,
	models: z.record(z.string(), FrozenBucketSchema),
	hours: z.record(z.string().regex(/^(?:[0-9]|1\d|2[0-3])$/), FrozenHourSchema),
	// Optional so archives written before workspace attribution stay readable.
	byWorkspace: z.record(z.string(), FrozenWorkspaceSchema).optional(),
	entryCount: z.number().int().nonnegative(),
 ...
 * `invalidRecords` (dsh-0.1.2-alpha.5+) moves a day row that fails its schema
 * aside and opens without it — usage data is derived and rebuildable, so one
 * corrupt record must not keep the whole domain unopenable; hosts without
 * the policy (and the alpha.3 `defineDomain` that ignores the unknown field)
 * keep the previous loud rejection.
 */
...
function buildRepository(domain) {
	...
	let closing = null;
	const enqueue = (job) => {
		if (closing !== null) return Promise.reject(new Error("usage-stats repository is closed"));
	...
	return {
		...
		/** Cheap monotone change token for live UI invalidation. */
		revision() {
			return revision;
		},
		...
		/** Reject new updates, drain accepted transforms, then release the host domain. */
		close() {
