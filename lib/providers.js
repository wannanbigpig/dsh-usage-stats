...
function parseZaiWindows(body) {
	...
	const byWindow = [...candidates].sort((left, right) => {
		...
		return (numberValue(left.item.nextResetTime) ?? Number.MAX_SAFE_INTEGER) - (numberValue(right.item.nextResetTime) ?? Number.MAX_SAFE_INTEGER);
	});
	const toWindow = (entry, kind) => ({
		kind,
		usedPercent: entry.usedPercent,
		remainingPercent: 100 - entry.usedPercent,
		resetsAt: resetTime(entry.item.nextResetTime),
...
const PROVIDER_ADAPTERS = Object.freeze({
	...
	"kimi-coding": { id: "kimi-coding", displayName: "Kimi Coding", official: true, apiKeyEnv: "KIMI_API_KEY", kind: "plan", defaultBaseURL: "https://api.kimi.com/coding", capabilities: [PROVIDER_CAPABILITIES.PLAN_QUOTA], query: queryKimi },
	minimax: { id: "minimax", displayName: "MiniMax", official: true, apiKeyEnv: "MINIMAX_API_KEY", kind: "plan", defaultBaseURL: "https://api.minimax.io/anthropic", capabilities: [PROVIDER_CAPABILITIES.PLAN_QUOTA], query: queryMiniMax },
	"minimax-cn": { id: "minimax-cn", displayName: "MiniMax 中国", official: true, apiKeyEnv: "MINIMAX_CN_API_KEY", kind: "plan", defaultBaseURL: "https://api.minimaxi.com/anthropic", capabilities: [PROVIDER_CAPABILITIES.PLAN_QUOTA], query: queryMiniMax },
	zai: { id: "zai", displayName: "Z.ai", official: true, apiKeyEnv: "ZAI_API_KEY", kind: "plan", defaultBaseURL: "https://api.z.ai/api/coding/paas/v4", capabilities: [PROVIDER_CAPABILITIES.PLAN_QUOTA], query: queryZai },
	// `zai-coding-cn` is a user-defined regional alias, not an official route.
	"zai-coding-cn": { id: "zai-coding-cn", displayName: "zai-coding-cn", official: false, apiKeyEnv: "ZAI_CODING_CN_API_KEY", kind: "plan", defaultBaseURL: "https://open.bigmodel.cn/api/coding/paas/v4", capabilities: [PROVIDER_CAPABILITIES.PLAN_QUOTA], query: queryZai }
});
