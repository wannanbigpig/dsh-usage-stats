...
import { costNanosOf, dayKey, defaultPricing, foldUsage, hourKey, isBilledModelKey, mergeInto, renderUsage, roundCost, tokenCountOf, totalTokens, zeroBuckets } from "./usage.js";
...
export function appendLedger(ledger, entry, options = {}) {
	...
	const normalized = {
		...
		...(typeof entry?.workspace === "string" && entry.workspace !== "" ? { workspace: entry.workspace } : {}),
...
export function pricingVersionOf(pricing = defaultPricing()) {
	...
	const canonical = JSON.stringify({
		...
		routes: [...(pricing?.routes ?? [])].map((route) => ({
			model: route?.model ?? null,
			priceModel: route?.priceModel ?? null,
			effectiveFrom: route?.effectiveFrom ?? null,
			effectiveUntil: route?.effectiveUntil ?? null
...
export function freezeLedgerEntry(entry, pricing = defaultPricing()) {
	...
	const billed = isBilledModelKey(modelKey);
	const costNanosCny = billed ? costNanosOf(modelKey, usage, hourKey(attributionAt), pricing, dayKey(attributionAt), attributionAt) : null;
	return {
		...(typeof entry?.id === "string" && entry.id !== "" ? { id: entry.id } : {}),
		occurredAt,
		...(Number(entry?.completedAt) > 0 ? { completedAt: Number(entry.completedAt) } : {}),
...
function archivedDayOf(archive, date) {
	...
	if (day === void 0) {
		day = { totals: zeroBuckets(), models: {}, hours: {}, byWorkspace: {}, entryCount: 0, pricingVersionCounts: {} };
...
/**
 * Workspace bucket inside one frozen day. Workspace totals are intentionally
 * NOT stored: they are derived by summing the sessions, so no layer can count
 * the same tokens twice.
 */
function archivedWorkspaceOf(day, workspace) {
	if (day.byWorkspace === void 0) day.byWorkspace = {};
	let entry = day.byWorkspace[workspace];
	if (entry === void 0) {
		entry = { sessions: {} };
		day.byWorkspace[workspace] = entry;
	}
	return entry;
}
...
export function foldFrozenLedgerEntries(entries, archive = createFrozenArchive()) {
	...
	for (const raw of entries ?? []) {
		...
		// Preserve the token attribution through compaction even though the
		// compacted archive no longer retains call identity.
		const archivedWorkspace = archivedWorkspaceOf(day, entry.workspace ?? "");
		let workspaceSession = archivedWorkspace.sessions[entry.sessionId ?? ""];
		if (workspaceSession === void 0) {
			workspaceSession = zeroBuckets();
			archivedWorkspace.sessions[entry.sessionId ?? ""] = workspaceSession;
		}
		addBuckets(workspaceSession, entry.usage);
		next.entryCount += 1;
...
export function mergeFrozenArchive(left, right) {
	...
	for (const [date, sourceDay] of Object.entries(source.days)) {
		...
		for (const [workspace, sourceWorkspace] of Object.entries(sourceDay.byWorkspace ?? {})) {
			const targetWorkspace = archivedWorkspaceOf(day, workspace);
			for (const [session, sourceBuckets] of Object.entries(sourceWorkspace.sessions ?? {})) {
				let target = targetWorkspace.sessions[session];
				if (target === void 0) {
					target = zeroBuckets();
					targetWorkspace.sessions[session] = target;
				}
				addBuckets(target, sourceBuckets);
			}
		}
...
export function recordLedgerState(state, entry, options = {}) {
	...
	return {
		...
		ledger,
		archive: {
			// A normal append/replacement cannot change frozen history. Retain
			// that branch instead of cloning every archived day on every call.
			frozen: exactOverflow.length === 0 ? current.archive.frozen : foldFrozenLedgerEntries(exactOverflow, current.archive.frozen),
			estimated: {
...
function trimWorkspaceEstimate(workspaces, cutoffDay) {
	const trimmed = {};
	for (const [workspace, entry] of Object.entries(workspaces ?? {})) {
		const sessions = {};
		for (const [session, sessionEntry] of Object.entries(entry?.sessions ?? {})) {
			const days = Object.fromEntries(Object.entries(sessionEntry?.days ?? {}).filter(([date]) => date >= cutoffDay));
			if (Object.keys(days).length > 0) sessions[session] = { days };
		}
		if (Object.keys(sessions).length > 0) trimmed[workspace] = { sessions };
	}
	return trimmed;
}
...
function trimEstimatedSource(source, cutoffDay) {
	...
	if (source.days === void 0) return days;
	const trimmed = { ...source, days };
	if (source.workspaces !== void 0) trimmed.workspaces = trimWorkspaceEstimate(source.workspaces, cutoffDay);
	return trimmed;
}

const WORKSPACE_UNKNOWN = "";

function workspaceSessionTarget(target, workspace, session) {
	let sessions = target.get(workspace);
	if (sessions === void 0) {
		sessions = new Map();
		target.set(workspace, sessions);
	}
	let days = sessions.get(session);
	if (days === void 0) {
		days = new Map();
		sessions.set(session, days);
	}
	return days;
}

function addWorkspaceBuckets(target, workspace, session, date, buckets) {
	const days = workspaceSessionTarget(target, workspace, session);
...
function inWorkspaceRange(date, from, to) {
...
/**
 * Merge the exact ledger/frozen workspace attribution with the
 * session-persistence rebuild into one date-filtered workspace to session
 * token view. The rebuild covers only pre-coverage events, so the exact and
 * estimated layers are disjoint and can be summed. Workspace totals are
 * derived from the session rows, so nothing is stored or returned twice.
 *
 * @param state - persisted v3 ledger state.
 * @param options - inclusive Beijing-calendar `from`/`to` bounds (YYYY-MM-DD).
 * @returns `{ from, to, tokens, inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens, workspaces, hasEstimatedHistory }`.
 */
export function renderWorkspaceUsage(state, options = {}) {
	const current = createLedgerState(state);
	const from = typeof options.from === "string" && options.from !== "" ? options.from : null;
	const to = typeof options.to === "string" && options.to !== "" ? options.to : null;
	const combined = new Map();
	for (const entry of current.ledger) {
		const at = Number(entry.completedAt) > 0 ? Number(entry.completedAt) : Number(entry.occurredAt) || 0;
		addWorkspaceBuckets(combined, entry.workspace ?? WORKSPACE_UNKNOWN, entry.sessionId ?? "", dayKey(at), entry.usage);
	}
	for (const [date, day] of Object.entries(current.archive.frozen.days ?? {})) {
		for (const [workspace, archivedWorkspace] of Object.entries(day.byWorkspace ?? {})) {
			for (const [session, buckets] of Object.entries(archivedWorkspace.sessions ?? {})) {
				addWorkspaceBuckets(combined, workspace, session, date, buckets);
			}
		}
	}
	const rebuild = current.archive.estimated?.sessionRebuild;
	for (const [workspace, archivedWorkspace] of Object.entries(rebuild?.workspaces ?? {})) {
		for (const [session, source] of Object.entries(archivedWorkspace.sessions ?? {})) {
			for (const [date, buckets] of Object.entries(source?.days ?? {})) {
				addWorkspaceBuckets(combined, workspace, session, date, buckets);
			}
		}
	}
	...
	const workspaces = [];
	for (const [workspace, sessions] of combined) {
		const workspaceBuckets = zeroBuckets();
		const sessionRows = [];
		for (const [session, days] of sessions) {
			const sessionBuckets = zeroBuckets();
			for (const [date, buckets] of days) {
				if (inWorkspaceRange(date, from, to)) addBuckets(sessionBuckets, buckets);
			}
			const tokens = totalTokens(sessionBuckets);
			if (tokens <= 0) continue;
			sessionRows.push({ sessionId: session, ...sessionBuckets, tokens });
			addBuckets(workspaceBuckets, sessionBuckets);
		}
		if (sessionRows.length === 0) continue;
		const tokens = totalTokens(workspaceBuckets);
		sessionRows.sort((left, right) => right.tokens - left.tokens || left.sessionId.localeCompare(right.sessionId));
		workspaces.push({ workspace, ...workspaceBuckets, tokens, sessions: sessionRows });
		addBuckets(total, workspaceBuckets);
	}
	workspaces.sort((left, right) => right.tokens - left.tokens || left.workspace.localeCompare(right.workspace));
	return {
		from,
		to,
		...total,
		...
		workspaces,
		hasEstimatedHistory: Object.values(current.archive.estimated ?? {}).some((source) => hasEstimatedSource(source) || Object.keys(source?.workspaces ?? {}).length > 0)
	};
}
