...
import { costOf, dayKey, defaultPricing, foldUsage, hourKey, isBilledModelKey, renderUsage, roundCost } from "./usage.js";
...
function applyFrozenCosts(rendered, ledger, pricing) {
	...
	for (const day of rendered.days) {
		...
		day.cost = costValue(costs, `d:${day.date}`) ?? day.cost;
		for (const model of day.models) model.cost = costValue(costs, `m:${day.date}:${model.model}`) ?? model.cost;
		for (const hour of day.hours) {
			hour.cost = costValue(costs, `h:${day.date}:${hour.hour}`) ?? hour.cost;
			for (const model of hour.models) model.cost = costValue(costs, `hm:${day.date}:${hour.hour}:${model.model}`) ?? model.cost;
		}
	}
