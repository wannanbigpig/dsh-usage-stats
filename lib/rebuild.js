...
import { dayKey, foldUsage, mergeInto } from "./usage.js";
 ...
 * (`list`/`open`) whose owned log reads after the fork-inherited prefix — and every
...
async function readThroughHandle(sessionPersistence, id, signal) {
	...
	try {
		...
		return await handle.read(handle.inheritedEventCount ?? 0, void 0, { signal });
	} finally {
		await handle.close();
	}
}
