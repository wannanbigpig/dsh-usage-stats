...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * `settings.section`. Hosts configuration that changes billing or call
			 * behavior: 供应商与账户 / 供应商用量与计费 / 通知与提示 / 折叠会话 / 数据管理.
		...
		function ConversationCard({ translate, conversation, onConversationUpdated }) {
			...
			const showTokenUsage = conversation?.showTokenUsage === true;
			const showSessionTokenUsage = conversation?.showSessionTokenUsage === true;
						...
						children: [
							...
							react_jsx_runtime.jsxs("div", {
								...
								react_jsx_runtime.jsxs("label", {
									...
									children: [
										...
										disabled: saving,
										onChange: (event) => saveConversation({ showTokenUsage: event.target.checked, ...(event.target.checked ? { showSessionTokenUsage: false } : {}) })
									}),
									react_jsx_runtime.jsx("span", { className: S.switchSlider })
									]
								})
							...
							react_jsx_runtime.jsxs("div", {
								...
								children: [
									...
									className: S.toggleInfo,
									children: [
										react_jsx_runtime.jsx("span", { className: S.toggleTitle, children: translate("conversation.showSessionTokenUsage") }),
										react_jsx_runtime.jsx("span", { className: S.toggleDesc, children: translate("conversation.showSessionTokenUsageDesc") })
									]
								}),
								react_jsx_runtime.jsxs("label", {
									className: S.switch,
									children: [
										react_jsx_runtime.jsx("input", {
										type: "checkbox",
										checked: showSessionTokenUsage,
										disabled: saving,
							onChange: (event) => saveConversation({ showSessionTokenUsage: event.target.checked, ...(event.target.checked ? { showTokenUsage: false } : {}) })
						}),
						react_jsx_runtime.jsx("span", { className: S.switchSlider })
						]
					})
		...
		function UsageBillingSettingsSection({ t }) {
			...
			const [conversation, setConversation] = react.useState({ enabled: true, showTokenUsage: false, showSessionTokenUsage: false });
		...
		function compactGroupFrom(order, store, start, end, partialKey) {
			...
			return {
				...
				failures: entries.filter((entry) => entry.kind === "tool" && entry.error).length,
				running,
				error: !running && latest.kind === "reasoning" && latest.error === true
					&& entries.every((entry) => entry.kind !== "tool" || entry.error !== true),
		...
		function compactIsTerminalProcessNode(node) {
			if (node?.kind === "turn-error" || node?.kind === "turn-max-tokens") return true;
			return node?.kind === "model-retry"
				&& (node.data?.terminal === true || node.data?.final === true || node.data?.status === "closed");
		}

		function compactIsProcessNode(node) {
			return node !== null && node !== undefined
				&& !compactIsConversationBoundary(node)
				&& node.kind !== "turn-tail"
				&& !compactIsTerminalProcessNode(node);
		}
		...
		function compactMutationRelevant(records) {
			return (records ?? []).some((record) => {
				// characterData mutations target Text nodes, which have no
				// Element.closest — an unchecked optional chain would make EVERY
				// unrelated text change look relevant and trigger full DOM scans.
				const target = record?.target;
				if (target?.nodeType === 1 && target.closest("[data-chat-flow]") !== null) return true;
				return [...(record?.addedNodes ?? []), ...(record?.removedNodes ?? [])].some((node) => mutationNodeMatches(node, "[data-chat-flow]"));
			});
		}
		...
		function compactTurnEndedWithError(turn) {
			var reasonKind = turn?.end?.data?.reason?.kind;
			return reasonKind === "error" || reasonKind === "limit" || reasonKind === "max-tokens" || reasonKind === "rate-limit";
		}
		...
		function compactProcessGroups(order, store, timeline, legacy) {
			...
			while (index < order.length) {
				...
				if (startNode === undefined || compactTurnOf(startNode) === null) {
					index++;
					continue;
				}
				var skippedLeadingBoundary = compactIsConversationBoundary(startNode);
				var turnNumber = compactTurnNumber(startNode);
				var turn = timeline?.turns?.get?.(turnNumber) ?? compactTurnOf(startNode);
				var turnNodes = [];
				while (index < order.length) {
					var candidate = compactNodeAt(store, order[index]);
					if (compactTurnNumber(candidate) !== turnNumber) break;
					turnNodes.push(candidate);
					index++;
				}
				// Session-start hooks may inject approval/context before the first
				// direct user message. Keep that preamble outside the response fold,
				// otherwise the marker is anchored above the question and expanding it
				// produces two visual sections around the still-visible user row. Later
				// steering boundaries remain visible without splitting this turn into
				// duplicate process groups. When the outer scan starts on the direct
				// user row, that leading boundary is already accounted for and must not
				// cause a later steering/user-message node to erase the earlier process.
				var firstBoundary = skippedLeadingBoundary ? -1 : turnNodes.findIndex(compactIsConversationBoundary);
				var segment = turnNodes.filter((candidate, candidateIndex) => {
					return compactIsProcessNode(candidate) && (firstBoundary < 0 || candidateIndex > firstBoundary);
				});
				...
				var running = !terminated && (!settled || entries.some((entry) => entry.running));
					var error = !hasFinalAnchor && !terminated && (compactTurnEndedWithError(turn) || turn?.status === "interrupted" || (!running && failures > 0));
		...
		/** Normalize Harness' whole-session token projection to the process metric shape. */
		function compactSessionTokenUsage(usage) {
			if (usage === null || typeof usage !== "object") return null;
			const read = (primary, fallback) => {
				let value = Number(usage[primary]);
				if (!Number.isFinite(value)) value = Number(usage[fallback]);
				return Number.isFinite(value) && value >= 0 ? value : 0;
			};
			return {
				inputTokens: read("uncachedInputTokens", "inputTokens"),
				cacheReadTokens: read("cacheReadTokens", "cacheRead"),
				cacheWriteTokens: read("cacheWriteTokens", "cacheWrite"),
				outputTokens: read("outputTokens", "output")
			};
		}
		...
		function compactSync(groups, processGroups, translate, showTokenUsage, showSessionTokenUsage, sessionTokenUsage, sessionKey) {
			var now = Date.now();
			var displayProcessGroups = processGroups.map((group, index) => {
				if (showSessionTokenUsage) {
					// The projection is session-wide; show it once on the latest fold rather
					// than repeating the same cumulative number on every historical turn.
					return { ...group, tokenUsage: index === processGroups.length - 1 ? sessionTokenUsage : null };
				}
				return { ...group, tokenUsage: showTokenUsage ? group.tokenUsage : null };
			});
		...
		function CompactConversationController(props) {
			...
			var sessionTokenUsage = typeof useProjection === "function" ? useProjection("tokenUsage") : undefined;
			...
			var settingsRef = react.useRef({ enabled: true, showTokenUsage: false, showSessionTokenUsage: false });
			...
			react.useEffect(function () {
				var onSettings = function (event) {
					...
					settingsRef.current = {
						...
						showTokenUsage: next.showTokenUsage === true,
						showSessionTokenUsage: next.showSessionTokenUsage === true
					};
					syncRef.current();
				};
			...
			react.useEffect(function () {
				syncRef.current = function () {
					...
					compactSync(groupsRef.current, processGroupsRef.current, t, settingsRef.current.showTokenUsage, settingsRef.current.showSessionTokenUsage, compactSessionTokenUsage(sessionTokenUsage), sessionKey);
				};
				syncRef.current();
			}, [groups, processGroups, t, sessionKey, sessionTokenUsage]);
			...
			react.useEffect(function () {
				...
				var observer = new MutationObserver(function (records) { if (compactMutationRelevant(records)) schedule(); });
		...
		const zh = {
			...
			"conversation.title": "会话显示设置",
			"conversation.desc": "将本轮模型处理过程收进一个外层折叠；Token 统计默认关闭，因为单轮统计基于当前聊天节点，可能受分步调用、实时上报、分页或压缩影响而不等于实际用量。",
			"conversation.enable": "折叠模型过程",
			"conversation.enableDesc": "开启后，本轮最终回复前的过程默认收进大折叠；展开后仍可单独操作思考和工具调用的小折叠，最终回复始终正常显示。",
			"conversation.showTokenUsage": "统计本轮 Token（近似）",
			"conversation.showTokenUsageDesc": "显示当前问答（本轮）的输入/输出 Token；这是基于当前聊天节点的近似值，可能与实际用量不一致。",
			"conversation.showSessionTokenUsage": "使用会话 Token 统计",
			"conversation.showSessionTokenUsageDesc": "使用与底部统计相同的 Harness 会话累计 Token，数据准确但不能拆分到单条消息或单个问答。开启后自动关闭本轮统计。",
		...
		const en = {
			...
			"conversation.title": "Conversation Display",
			"conversation.desc": "Fold the model's process for each turn into one outer item. Token counts are off by default because turn counts are inferred from the current chat nodes and may differ from actual usage due to steps, live updates, paging, or compaction.",
			"conversation.enable": "Fold model process",
			...
			"conversation.showTokenUsage": "Count turn tokens (approx.)",
			"conversation.showTokenUsageDesc": "Show input/output tokens for the current question and answer. This is inferred from the current chat nodes and may differ from actual usage.",
			"conversation.showSessionTokenUsage": "Use session token totals",
			"conversation.showSessionTokenUsageDesc": "Use the accurate Harness session-wide token total shown at the bottom. It cannot be split by individual message or turn. Enabling it disables turn counts.",
		...
		exports.compactActivityGroups = compactActivityGroups;
		exports.compactMutationRelevant = compactMutationRelevant;
