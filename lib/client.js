...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".usg_dayValueBar{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,color-mix(in srgb,var(--usg-blue) 60%,#85a8ff),var(--usg-blue));box-shadow:0 0 8px color-mix(in srgb,var(--usg-blue) 25%,transparent)}",
		 ...
		 */
		function WorkspaceUsageView({ translate, serverToday, range = "7d", customFrom = null, customTo = null, animationKey = 0 }) {
			...
			react.useEffect(() => { load(); }, [load, animationKey]);
			...
			return react_jsx_runtime.jsxs("div", {
				...
				children: [
					...
					error !== null
						? react_jsx_runtime.jsx("p", { className: S.error, children: translate("workspace.error", { message: error }) })
		...
		function UsageStatsSection({ t, onClose, onOpenSettings }) {
			...
			const filteredDays = react.useMemo(() => days.map(day => filterDay(day, activeModelFilter)), [usage, activeModelFilter]);
				...
				react.useEffect(() => {
					if (modelFilter !== "" && !modelChoices.some((item) => item.id === modelFilter)) setModelFilter("");
				}, [modelFilter, modelChoices]);
