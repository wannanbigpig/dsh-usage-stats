...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".usg_usageLimitBanner[data-status=warning]{background:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 12%,transparent);border-color:rgba(128,128,128,.22)!important}",
		...
		function sidebarSummaryOf(usagePayload, balancePayload, limitsPayload, today, providerId = "deepseek-official", providerLabel = null) {
		...
		function BalanceCard({ keys, providers = [], selectedProviderId, selectedKey, onSelectKey, account, accountLoading, accountError, balanceTone = "muted", translate, onRetry }) {
			...
			return react_jsx_runtime.jsxs("section", {
				...
				children: [
					react_jsx_runtime.jsxs("div", {
						...
						children: [
							...
							react_jsx_runtime.jsxs("span", {
								...
								children: [
									react_jsx_runtime.jsx("span", { className: S.balanceLabel, children: selectedProviderId && providers.length > 0
									? `${providers.find((item) => item.id === selectedProviderId)?.label ?? selectedProviderId} ${translate("balance.providerSuffix")}`
					...
					windows.length > 0 && react_jsx_runtime.jsxs("div", {
						className: S.balanceRows,
						"data-usage-plan-windows": true,
						children: windows.map((item, index) => {
							const remainingPercent = Number.isFinite(Number(item.remainingPercent)) ? Number(item.remainingPercent) : null;
							const amount = remainingPercent !== null
								? `${remainingPercent.toFixed(0)}% ${translate("balance.remaining")}`
								: item.limit !== void 0 || item.remaining !== void 0 ? `${fmt(item.remaining ?? 0)} / ${fmt(item.limit ?? 0)}` : "—";
							return react_jsx_runtime.jsxs("div", {
								className: S.balanceRow,
								children: [react_jsx_runtime.jsx("span", { children: windowLabel(item.kind) }), react_jsx_runtime.jsx("b", { children: amount })]
		...
		function UsageStatsSection({ t, onClose, onOpenSettings }) {
			...
			}, [loadUsage, loadKeys, loadProviders]);
			react.useEffect(() => {
				setSelectedKey((current) => {
		...
		function UsageBillingSettingsSection({ t }) {
			...
			const loadProviders = react.useCallback(() => {
				return fetchJson("/api/usage-stats/providers").then((payload) => {
					...
					if (payload.ok !== true || list.length === 0) throw new Error("provider endpoint unavailable");
					setProviders(list);
					setDefaultProviderId(String(payload.defaultProviderId ?? "deepseek-official"));
				}).catch(() => fetchJson("/api/usage-stats/keys").then((payload) => {
					const list = payload.ok === true && Array.isArray(payload.keys) ? payload.keys : [];
					setProviders([legacyDeepseekProvider(list)]);
		...
		function UsageStatsPanel({ wide, t }) {
			...
			const balanceLabel = summary.providerLabel ? `${summary.providerLabel} ${t("panel.balance")}` : t("panel.balance");
			const summaryText = [
				showBalance ? `${balanceLabel} ${summary.balance}` : null,
				showToday ? `${t("panel.today")} ${summary.today}` : null
		...
		const zh = {
			...
			"balance.windowFiveHour": "5 小时窗口",
			"balance.windowWeekly": "每周窗口",
		...
		const en = {
			...
			"balance.windowFiveHour": "5-hour window",
			"balance.windowWeekly": "Weekly window",
