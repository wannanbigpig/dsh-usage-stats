...
// The timeout covers response-body parsing as well as the initial fetch.
{
	const result = await queryProvider("deepseek-official", {
		apiKey: "test-api-key",
		timeoutMs: 5,
		fetchImpl: async () => ({
			status: 200,
			ok: true,
			json: async () => { await new Promise((resolve) => setTimeout(resolve, 30)); return { balance_infos: [{ currency: "CNY", total_balance: "1" }] }; }
		})
	});
	check(statusOf(result) === "timeout", "slow response bodies must honor the provider timeout");
}

async function query(providerId, fetchImpl, extra = {}) {
	return providerResult(await queryProvider(providerId, {
		apiKey: "test-api-key",
		managementApiKey: extra.managementApiKey,
...
// model route, but must be rendered as unsupported instead of being queried.
{
	...
	const xiaomiApi = builtins.find((provider) => provider.id === "xiaomi");
	const zai = builtins.find((provider) => provider.id === "zai-coding-cn");
	const moonshot = builtins.find((provider) => provider.id === "moonshotai-cn");
	const opencode = builtins.find((provider) => provider.id === "opencode-go");
	const opencodeZen = builtins.find((provider) => provider.id === "opencode");
	const kimi = builtins.find((provider) => provider.id === "kimi-coding");
	const openrouter = builtins.find((provider) => provider.id === "openrouter");
	...
	check(xiaomi?.accountUrl === "https://platform.xiaomimimo.com/console/plan-manage", "Xiaomi unsupported quota metadata exposes only its official web query entry");
	check(xiaomiApi?.queryable === false && xiaomiApi?.accountUrl === "https://platform.xiaomimimo.com/console/usage", "Xiaomi API-key route exposes its web-only usage page");
	...
	check(moonshot?.apiKeyEnv === "MOONSHOTAI_CN_API_KEY" && moonshot?.capabilities?.includes("balance"), "Moonshot CN reuses the Harness model credential ref and advertises balance");
	check(JSON.stringify(opencode?.planQuota?.windows) === JSON.stringify(["five_hour", "weekly", "monthly"]) && opencode?.accountUrl === void 0, "OpenCode Go metadata declares all official usage windows without a redundant web query entry");
	check(opencodeZen?.queryable === false && opencodeZen?.accountUrl === "https://opencode.ai/workspace/wrk_01KN1AVY46S9P0AQNE6GNGNQE0", "OpenCode Zen remains web-only and exposes the configured workspace usage page");
	check(kimi?.accountUrl === "https://www.kimi.com/membership/subscription?tab=quota", "Kimi exposes the official web-only monthly quota page without adding a web-session adapter");
	check(openrouter?.managementKeyEnv === "OPENROUTER_MANAGEMENT_KEY" && openrouter?.capabilities?.includes("key_limit"), "OpenRouter declares its separate Management Key and normal-key limit capability");
	check(openrouter?.extraCredentials?.[0]?.id === "managementApiKey" && openrouter?.extraCredentials?.[0]?.ref === "OPENROUTER_MANAGEMENT_KEY", "OpenRouter exposes a write-only Management Key field for provider-scoped settings");
	let called = false;
...
// Moonshot/Kimi Open Platform balance uses the model API key directly. The
// regional endpoint determines the currency because CN and global keys are
// deliberately not interchangeable.
{
	const { fetchImpl, calls } = mockFetch([{
		match: (url) => url === "https://api.moonshot.cn/v1/users/me/balance",
	...
	const result = await query("moonshotai-cn", fetchImpl, { baseURL: "https://api.moonshot.cn/v1" });
	check(statusOf(result) === "ok", "Moonshot CN balance succeeds");
	check(balanceOf(result)?.currency === "CNY" && balanceOf(result)?.total === 49.58894, "Moonshot CN balance and currency are normalized");
	check(balanceOf(result)?.granted === 46.58893 && balanceOf(result)?.toppedUp === 3.00001, "Moonshot voucher and cash balances map to provider-neutral fields");
	check(calls.length === 1 && authHeader(calls[0].init) === "Bearer test-api-key", "Moonshot uses Bearer authentication with the model API key");
}
...
// OpenRouter's ordinary model key always queries /key and must never be sent
// to the Management-Key-only /credits endpoint.
{
	const { fetchImpl, calls } = mockFetch([{
		match: (url) => url === "https://openrouter.ai/api/v1/key",
		response: jsonResponse(200, { data: { limit: 100, limit_remaining: 74.5, usage: 25.5, limit_reset: "monthly" } })
	}]);
	const result = await query("openrouter", fetchImpl, { baseURL: "https://openrouter.ai/api/v1" });
	check(statusOf(result) === "ok", "OpenRouter ordinary key-limit query succeeds");
	check(calls.length === 1 && calls[0].url.endsWith("/api/v1/key"), "OpenRouter does not call credits without a Management Key");
	check(result.keyLimit?.limit === 100 && result.keyLimit?.remaining === 74.5 && result.keyLimit?.used === 25.5, "OpenRouter ordinary key limit is normalized separately from account credits");
	check(balanceOf(result) === null, "OpenRouter does not fabricate account balance from a normal key limit");
}

// When separately configured, only the Management Key is sent to /credits;
// both snapshots remain independently visible in one provider result.
{
	const { fetchImpl, calls } = mockFetch([
		{ match: (url) => url === "https://openrouter.ai/api/v1/key", response: jsonResponse(200, { data: { limit: 100, limit_remaining: 74.5, usage: 25.5 } }) },
		{ match: (url) => url === "https://openrouter.ai/api/v1/credits", response: jsonResponse(200, { data: { total_credits: "100", total_usage: "12.5" } }) }
	]);
	const result = await query("openrouter", fetchImpl, { baseURL: "https://openrouter.ai/api/v1", managementApiKey: "test-management-key" });
	check(statusOf(result) === "ok" && result.managementStatus === "ok", "OpenRouter combines successful key-limit and credits snapshots");
	check(calls.length === 2 && authHeader(calls.find((call) => call.url.endsWith("/credits"))?.init) === "Bearer test-management-key", "OpenRouter credits uses only the separate Management Key");
	check(balanceOf(result)?.total === 87.5 && balanceOf(result)?.remaining === 87.5 && balanceOf(result)?.purchased === 100 && balanceOf(result)?.used === 12.5, "OpenRouter management credits are normalized as remaining account balance");
}

// A bad optional Management Key must not erase a valid ordinary-key snapshot.
{
	const { fetchImpl } = mockFetch([
		{ match: (url) => url.endsWith("/api/v1/key"), response: jsonResponse(200, { data: { limit: 20, limit_remaining: 8, usage: 12 } }) },
		{ match: (url) => url.endsWith("/api/v1/credits"), response: jsonResponse(403, { error: { message: "forbidden" } }) }
	]);
	const result = await query("openrouter", fetchImpl, { managementApiKey: "bad-management-key" });
	check(statusOf(result) === "ok" && result.managementStatus === "unauthorized", "OpenRouter preserves key limits and reports Management Key failure independently");
	check(result.keyLimit?.remaining === 8 && balanceOf(result) === null, "OpenRouter partial success contains no fabricated credits balance");
}

// OpenCode Go exposes subscription windows through the same model API key.
{
	const { fetchImpl, calls } = mockFetch([{
		match: (url) => url === "https://opencode.ai/zen/go/v1/usage",
		response: jsonResponse(200, { usage: {
			rolling: { status: "ok", percent: 12, resetsAt: "2026-08-25T12:00:00.000Z" },
			weekly: { status: "ok", percent: 34, resetsAt: "2026-08-30T00:00:00.000Z" },
			monthly: { status: "rate-limited", percent: 100, resetsAt: "2026-09-01T00:00:00.000Z" }
		} })
	}]);
	const result = await query("opencode-go", fetchImpl, { baseURL: "https://opencode.ai/zen/go/v1" });
	check(statusOf(result) === "ok", "OpenCode Go usage query succeeds");
	check(JSON.stringify(windowsOf(result).map((item) => item.kind)) === JSON.stringify(["five_hour", "weekly", "monthly"]), "OpenCode Go normalizes rolling usage as a five-hour window");
	check(windowOf(result, ["five_hour"])?.usedPercent === 12 && windowOf(result, ["monthly"])?.remainingPercent === 0, "OpenCode Go normalizes used and remaining percentages");
	check(calls.length === 1 && authHeader(calls[0].init) === "Bearer test-api-key", "OpenCode Go uses the model API key as Bearer authentication");
}

// A valid workspace key without a Go entitlement is different from an invalid
// credential and should tell the UI that a subscription is required.
{
	const { fetchImpl } = mockFetch([{
		match: (url) => url === "https://opencode.ai/zen/go/v1/usage",
		response: jsonResponse(403, { type: "error", error: { type: "EntitlementError", message: "OpenCode Go subscription required." } })
	}]);
	const result = await query("opencode-go", fetchImpl);
	check(statusOf(result) === "not-subscribed", "OpenCode Go entitlement failures are distinct from invalid API keys");
}

{
	const { fetchImpl } = mockFetch([{
		match: (url) => url === "https://opencode.ai/zen/go/v1/usage",
		response: jsonResponse(200, { usage: { monthly: { status: "ok", percent: 10, resetsAt: "2026-09-01T00:00:00.000Z" } } })
	}]);
	const result = await query("opencode-go", fetchImpl);
	check(statusOf(result) === "invalid-response", "OpenCode Go rejects incomplete official window snapshots");
}
...
// A successful fallback response without quota data is an invalid response,
// not the status of the earlier failed endpoint.
{
	const result = await query("minimax-cn", async (url) => {
		if (url.endsWith("/v1/token_plan/remains")) return jsonResponse(429, {});
		return jsonResponse(200, { data: { model_remains: [] } });
	});
	check(statusOf(result) === "invalid-response", "MiniMax empty fallback must report invalid response");
}
...
// A syntactically valid data envelope without any quota windows is not a
// successful live snapshot; callers must keep the provider unavailable.
{
	const { fetchImpl } = mockFetch([{
		match: (url) => url === "https://api.z.ai/api/monitor/usage/quota/limit",
		response: jsonResponse(200, { data: { limits: [] } })
	}]);
	const result = await query("zai", fetchImpl, { baseURL: "https://api.z.ai/api/coding/paas/v4" });
	check(statusOf(result) === "invalid-response" && windowsOf(result).length === 0, "Z.ai empty quota data must not be reported as ok");
}
...
// A timeout while reading the response body is still an unavailable provider.
{
	const timeoutError = new Error("body timed out");
	timeoutError.name = "TimeoutError";
	await assert.rejects(
		queryDeepSeekBalance("https://api.deepseek.com", "test-api-key", 10, async () => ({
			ok: true,
